...
- PostgreSQL differs by environment: development rejects TLS, while published apps require it. Detect `REPLIT_DEPLOYMENT === '1'` and enable `pg` SSL only there.
  **Why:** an Autoscale build succeeded but promotion crash-looped with `connection is insecure (try using sslmode=require)`, while forcing SSL in development had previously failed.
  **How to apply:** keep conditional SSL whenever database connection configuration changes; do not use one fixed SSL mode for both environments.
